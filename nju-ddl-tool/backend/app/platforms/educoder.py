...
class EducoderAdapter(PlatformAdapter):
    ...
    async def _fetch_target_assignments(
        ...
        for hw in captured:
            ...
            try:
                ...
                logger.warning("Educoder assignment normalization failed for %s: %s", assignment_id, exc)
        return items
    ...
    async def _fetch_json(self, page, url: str):
        try:
            ...
            logger.warning("Educoder JSON fallback failed for %s: %s", url, exc)
            return None
    ...
    def _parse_datetime(value: object | None) -> datetime | None:
        ...
        text = value.strip()
        if text.isdigit():
            return EducoderAdapter._parse_datetime(int(text))
        for fmt in (
            "%Y-%m-%d %H:%M:%S",
            "%Y-%m-%d %H:%M",
            "%Y-%m-%dT%H:%M:%S%z",
